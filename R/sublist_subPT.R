...
df.inquiry_sub <- df.inquiry_sub %>% group_by(req_id, request_type) %>% summarise(sub = n_distinct(req_quantity)) 

plot_task1 <- ggplot(df.inquiry_sub, aes(x = sub)) +
...
sum_stats_sub <- df.inquiry_sub %>% group_by(request_type) %>%
  summarise(
